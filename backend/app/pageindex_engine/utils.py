...
try:
    ...
    PyPDF2 = None  # PDF mode not available — markdown mode only
import copy
import asyncio
try:
    import pymupdf
except ImportError:
    pymupdf = None  # PDF mode not available — markdown mode only
...
from app.llm_provider import get_sync_client, get_async_client
...
def count_tokens(text, model=None):
    ...
    try:
        ...
        # Unknown model name (e.g. deepseek-chat) — fall back to cl100k_base
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print(f"Error parsing JSON from string: {inner_e}")
                return []
    except Exception as e:
...
def remove_fields(data, fields=None):
    """
    Recursively removes specified fields from a dictionary or list.
    """
    if fields is None:
        fields = ['text']
    if isinstance(data, dict):
